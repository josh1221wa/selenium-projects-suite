...
prices = [property.find(name="span").getText().split("+")[0].split("/")[0].replace(",", '') for property in
          properties]
urls = [property.find(name="a", class_="property-card-link").get('href')
...
for property in range(len(properties)):
    ...
    form_fields = [input for input in driver.find_elements(
        by=By.TAG_NAME, value="input") if input.get_attribute("type") == "text"]
    form_fields[0].send_keys(addresses[property])
    form_fields[1].send_keys(prices[property])
    form_fields[2].send_keys(urls[property])
    driver.find_element(
        by=By.XPATH, value="/html/body/div/div[2]/form/div[2]/div/div[3]/div[1]/div[1]/div/span").click()
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(
        (By.PARTIAL_LINK_TEXT, 'Submit another')))
