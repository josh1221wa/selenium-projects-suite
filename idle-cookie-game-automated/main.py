...
while True:
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(
        (By.CSS_SELECTOR, "#store div")))    # To avoid stale elements
    powerups = [driver.find_element(
        by=By.XPATH, value=f'//*[@id="store"]/div[{n}]') for n in range(1, 9)]
    powerups.pop()  # Last powerup is hidden by developer
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(
        (By.CSS_SELECTOR, "#store div b")))  # TO avoid stale elements
    prices = [int(price.text.split("-")[1].strip().replace(",", ""))
              for price in driver.find_elements(by=By.CSS_SELECTOR, value="#store div b")[:-1]]
