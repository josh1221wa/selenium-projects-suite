...
WebDriverWait(driver, 10).until(EC.presence_of_element_located(
    (By.XPATH, '//*[@id="main"]/div/div[1]/div/ul/li[1]')))
jobs = [driver.find_element(
    by=By.XPATH, value=f'//*[@id="main"]/div/div[1]/div/ul/li[{n}]') for n in range(1, 26)]
...
for job in jobs:
    ...
    try:
        ...
        driver.execute_script('arguments[0].click()', driver.find_element(
            by=By.CLASS_NAME, value="jobs-apply-button"))
    ...
    [number.send_keys(9876543210)
     for number in inputs if "phoneNumber-nationalNumber" in str(number.get_attribute("class"))]
    if "Submit application" in [button.text for button in driver.find_elements(by=By.TAG_NAME, value='button')]:
        driver.execute_script('arguments[0].click()', driver.find_element(
            by=By.XPATH, value="/html/body/div[3]/div/div/div[2]/div/div/form/footer/div[3]/button"))
    else:
        driver.execute_script('arguments[0].click()', driver.find_element(
            by=By.CLASS_NAME, value='artdeco-button--2'))
        driver.execute_script('arguments[0].click()', driver.find_element(
            by=By.CLASS_NAME, value='artdeco-button--2'))
        driver.execute_script('arguments[0].click()', driver.find_element(
            by=By.CLASS_NAME, value='artdeco-button'))
        save_button = driver.find_elements(
            by=By.CSS_SELECTOR, value=".artdeco-modal__actionbar--confirm-dialog button")[1]
